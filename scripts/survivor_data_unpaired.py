...
def var_0813():
	...
	print(f'Surviving firms in tdzs: {len(t2013[t2013["tdz"]>0])} vs non tdz: {len(t2013[t2013["tdz"]==0])}')
	...
	t2013.rename(columns={"l1": "tot_emp", "b2b": "perc_foriegn_own",
							...
							"k1c": "input_credit", "_2008_2013_ECAq69": "lf_ed"
							}, inplace=True)


	# Labor force education
	t2013l=t2013[t2013["lf_ed"]>0]
	t2013l["lf_ed"]=pd.to_numeric(t2013["lf_ed"], errors='coerce').dropna()
	print(f'labor force ed: {len(t2013l)}')

	# Total sales last year
	t2013s=t2013[t2013["tot_sales"]>0]
	t2013s["tot_sales"]=pd.to_numeric(t2013s["tot_sales"], errors='coerce').dropna()
	print(len(t2013s))

	# % inputs bought on credit
	t2013["input_credit"]=pd.to_numeric(t2013["input_credit"], errors="coerce").dropna()
	t2013=t2013[t2013["input_credit"]>0]
	print(len(t2013))

	col_ls = t2013.columns.tolist()
	t2013=t2013.iloc[:, [col_ls.index("idstd"), col_ls.index("tot_emp"),
						col_ls.index("year"),
	...
	col_ls = t2013s.columns.tolist()
	t2013s=t2013s.iloc[:, [col_ls.index("idstd"), col_ls.index("tot_emp"),
						col_ls.index("year"),
						col_ls.index("tot_sales"),
						col_ls.index("email"), col_ls.index("website"),
						col_ls.index("electricity"),
	...
	col_ls = t2013l.columns.tolist()
	t2013l=t2013l.iloc[:, [col_ls.index("idstd"), col_ls.index("tot_emp"),
						col_ls.index("year"),
						# col_ls.index("tot_sales"),
						col_ls.index("email"), col_ls.index("website"),
						col_ls.index("electricity"),
	...
	t2013.to_csv("./data_files/final_data/2013survivorscredit.csv")
	t2013s.to_csv("./data_files/final_data/2013survivorsales.csv")
	t2013l.to_csv("./data_files/final_data/2013survivorseducation.csv")
